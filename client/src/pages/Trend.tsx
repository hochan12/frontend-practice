...
const introText = `
매해 초, 인스타 매거진이나 유튜브 등 패션 관련 컨텐츠에서 빠짐없이 등장하는 올해 트렌드 예측.
...
const sections: TrendSection[] = [
  {
    title: "남성복도 바지핏은 좁아지고 상의 또한 타이트해졌다.",
    images: [
      { src: img1, alt: "trend example 1" },
      { src: img14, alt: "trend example 14" },
      { src: img3, alt: "trend example 3" },
      { src: img15, alt: "trend example 15" },
      { src: img4, alt: "trend example 4" },
      { src: img16, alt: "trend example 16" },
    ],
    leadAfterImages: "더하는 게 아닌 덜어내는 것에 집중하는 분위기.",
    description:
  ...
  {
    title: "마찬가지로 덜어내는 여성복",
    images: [
      { src: img6, alt: "trend example 6" },
      { src: img7, alt: "trend example 7" },
      { src: img8, alt: "trend example 8" },
      { src: img9, alt: "trend example 9" },
    ],
    leadAfterImages: "이러한 스타일링은 여성복에서도 확인할 수 있다.",
    description:
      "개인적으로 올해는 실루엣만으로 고급스러운 분위기를 자아내는 착장에 대한 선호가 더 많아질 것다. 덜어내지만 심심해 보이지는 않게 하는 데에 있어서 포인트는 사람 자체의 분위기에 옷을 녹아들게 하는 것이라고 생각한다. 무조건 잘생기거나 예쁠 필요도 없고 그저 본인 스타일에 대한 이해와 그에 맞는 분위기만 신경써줘도 착장이 한 층 더 매력있어질 것이다.",
    extraDescription:
      "2026년 트렌드 키워드로 포엣(poet) 코어를 많이 언급하곤 하는데 이름에서 알 수 있듯이 시인처럼 지적이고 감성적인 이미지의 룩을 생각하면 쉽다. 악세서리 또한 전체적으로 안경이나 가방처럼 과하지 않은 아이템들로 담백하게 스타일링하는 느낌이다.",
    extraImages: [
      { src: img18, alt: "extra 1" },
      { src: img20, alt: "extra 2" },
    ],
    leadAfterExtraImages: "데님과 니트, 셔츠를 활용한 자연스러운 분위기.",
  },
  {
    title: "또 다른 키워드, 모리걸",
    images: [
      { src: img10, alt: "trend example 10" },
      { src: img11, alt: "trend example 11" },
      { src: img12, alt: "trend example 12" },
    ],
    description:
      "일본어로 숲을 뜻하는 모리(森)와 소녀(girl)가 합쳐진 단어 모리걸, 약간의 빈티지함과 몽환적인 분위기를 지닌 스타일이라고 보면 좋을 것 같다. 이름에서 알 수 있듯이 일본 문화의 영향을 받은 느낌이 있는데, 색감을 다양하게 쓰거나 키치한 그래픽을 넣어 러블리한 감성을 만들어낸다.\n이러한 착장은 작년에 이어 올해도 더 다양한 브랜드에서 제품이 나오고 소비자들에게도 인기가 많을 것이라 생각된다.\n",
    extraImages: [
      { src: img13, alt: "extra 1" },
      { src: img17, alt: "extra 2" },
    ],
    leadAfterExtraImages:
      "전체적으로 비슷한 톤의 색감들로 매치하고, 프릴 스커트나 패턴이 있는 아이템들을 레이어드 해주면 수월하게 스타일링이 가능하다. 앞서 소개한 포엣 코어와는 다른 느낌의 빈티지한 느낌의 안경이나 악세서리를 사용해주는 것도 좋은 방법이다.",
    extraDescription:
